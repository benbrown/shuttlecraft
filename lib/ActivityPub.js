...
export class ActivityPubClient {
  ...
  async send(recipient, message) {
    ...
    let response = null;
    try {
      response = await fetch(recipient.inbox, {
        headers: {
          Host: url.hostname,
  ...
  validateSignature(actor, req) {
    ...
    try {
      if (actor) {
        ...
        verifier.update(str);
        return verifier.verify(actor.publicKey.publicKeyPem, signature.signature);
      } else {
        return false;
      }
    } catch (err) {
