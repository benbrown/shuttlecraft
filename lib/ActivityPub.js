...
export class ActivityPubClient {
  ...
  async webfinger(username) {
    let {
      targetDomain
    } = this.getUsernameDomain(username)
  ...
  async fetch(targetUrl, options) {
    ...
    let d = new Date();
    let stringToSign = `(request-target): get ${urlFragment}\nhost: ${url.hostname}\ndate: ${d.toUTCString()}`;
    signer.update(stringToSign);
    signer.end();
    const signature = signer.sign(this.account.privateKey);
    const signature_b64 = signature.toString('base64');
    let header = `keyId="${this.actor.publicKey.id}",headers="(request-target) host date",signature="${signature_b64}"`;
    options.headers = {
      'Date': d.toUTCString(),
      'Host': url.hostname,
  ...
  async send(recipient, message) {
    ...
    const signature = signer.sign(this.account.privateKey);
    const signature_b64 = signature.toString('base64');
    let header = `keyId="${this.actor.publicKey.id}",headers="(request-target) host date digest",signature="${signature_b64}"`;
    ...
    const res = await fetch(recipient.inbox, {
      headers: {
        'Host': url.hostname,
  ...
  /**
   * Send an outbound activity to a follower or recipient of a message
   * @param actor
   * @param message
   * @param recipients
   * @param followers
  ...
  async deliverOutbound(
    ...
    await Promise.all(
      recipients.map(async (recipient) => {
        ...
        if (recipient === actor.followers) {
          await Promise.all(
            followers.map(async (follower) => {
              await ActivityPub.send(follower, message);
            })
          );
        } else {
          // otherwise, send it directly to the person
          await ActivityPub.send(primaryRecipient, message);
        }
      })
    );
  }
  ...
  async sendBoost(primaryRecipient, post, followers) {
    ...
    await this.deliverOutbound(this.actor, message, recipients, followers, primaryRecipient);
  ...
  getUsername(userIdorName) {
    ...
    const {
      username,
      targetDomain
    } = this.getUsernameDomain(userIdorName);
        ...
        if (actorQuery.ok) {
          ...
          if (rootOutbox.first) {
            if (typeof rootOutbox.first == 'string') {
              ...
              const url = new URL(rootOutbox.first);
              const pageQuery = await ActivityPub.fetch(rootOutbox.first, {});
              if (pageQuery.ok) {
                outboxPage = await pageQuery.json();
                items = outboxPage.orderedItems || [];
              } else {
                logger('failed to load outbox first page', rootOutbox.first, pageQuery.status, pageQuery.statusText, await pageQuery.text());
              }
            } else {
          ...
          logger('failed to load outbox index', actor.outbox, actorQuery.status, actorQuery.statusText, await actorQuery.text());
        }
      } catch (err) {
        console.error(err);
      }
