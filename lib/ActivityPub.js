...
export class ActivityPubClient {
  ...
  async fetch(targetUrl, options) {
    ...
    const header = `keyId="${this.actor.publicKey.id}",headers="(request-target) host date",signature="${signature_b64}"`;
    options.headers = {
      'Date': d.toUTCString(),
      'Host': url.hostname,
  ...
  async send(recipient, message) {
    ...
    const signature = signer.sign(this.account.privateKey);
    const signature_b64 = signature.toString('base64');
    const header = `keyId="${this.actor.publicKey.id}",headers="(request-target) host date digest",signature="${signature_b64}"`;
