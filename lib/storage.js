  ...
  return new Promise((resolve, reject) => {
    glob(path.join(pathToFiles, '**/*.json'), async (err, files) => {
      ...
      for (const f of files) {
        try {
          const post = JSON.parse(fs.readFileSync(path.resolve(pathToFiles, f)).toString());
...
export const searchKnownUsers = async query => {
  return new Promise((resolve, reject) => {
    glob(path.join(pathToUsers, '**/*.json'), async (err, files) => {
      ...
      for (const f of files) {
        try {
          const user = JSON.parse(fs.readFileSync(path.resolve(pathToUsers, f)).toString());
          if (
