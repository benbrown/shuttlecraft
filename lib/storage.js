...
export const writeMediaFile = (filename, attachment) => {
  ...
  fs.writeFileSync(path.join(pathToMedia, filename), attachment.data);
...
export const readMediaFile = (filename) => {
  ...
  let attachment = JSON.parse(fs.readFileSync(path.join(pathToMedia, filename) + '.json'));
  attachment.data = fs.readFileSync(path.join(pathToMedia, filename));
  return attachment;
}
