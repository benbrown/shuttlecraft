  ...
  return new Promise((resolve, reject) => {
      glob(path.join(pathToUsers, '**/*.json'), async (err, files) => {
          ...
          try {
              ...
              if (user.actor?.id?.toLower().includes(query) || 
                  user.actor?.preferredUsername?.toLower().includes(query) ||
                  user.actor?.name?.toLower().includes(query) || 
                  user.actor?.url?.toLower().includes(query)) {
