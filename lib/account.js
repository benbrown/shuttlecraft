...
import { fetchOutbox, fetchUser, sendPost, sendToUser } from './users.js';
import { INDEX, readJSONDictionary, writeJSONDictionary, followersFile, followingFile, blocksFile, notificationsFile, likesFile, accountFile, createFileName, getFileName, boostsFile } from './storage.js';
import { getActivity, createActivity } from './notes.js';
...
/* send object directly, not a create */
export const sendActivityToFollowers = async(object) => {

  const followers = await getFollowers();
  followers.forEach(follower => {
    sendToUser(follower, object);
  });
