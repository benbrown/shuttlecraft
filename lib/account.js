...
import { createActivity, deleteActivity, getActivity } from './notes.js';
...
export const getInboxIndex = () => {
  ...
  return readJSONDictionary(inboxIndexPath, [{}]);
};
...
export const getInbox = actorId => {
  ...
  return readJSONDictionary(inboxPath, []);
};
...
export const addressedOnlyToMe = activity => {
  ...
  return addresses.length === 1 && addresses[0] === actor.id;
};
...
export const sendCreateToFollowers = async object => {
  ...
  for (const follower of followers) {
    ...
    try {
      await ActivityPub.sendCreate(account.actor, object);
    } catch (err) {
      // TODO: should retry
...
export const sendUpdateToFollowers = async object => {
  ...
  for (const follower of followers) {
    const account = await fetchUser(follower);
    try {
      await ActivityPub.sendUpdate(account.actor, object);
    } catch (err) {
      // TODO: should retry
      console.error('Failed to deliver outbound post', err);
    }
...
export const createNote = async (body, cw, inReplyTo, toUser, editOf) => {
  ...
  if (inReplyTo || toUser) {
    if (toUser) {
      ...
      // get parent post, so we can get the author and add to `TO` list
  ...
  for (const recipient of to.concat(cc)) {
    ...
    if (recipient === publicAddress) {
      ...
      if (editOf) {
        await sendUpdateToFollowers(object);
      } else {
        await sendCreateToFollowers(object);
      }
    } else {
...
export const filterFollowing = async () =>
  (
    await Promise.all(
...
export const filterFollowers = async following =>
  (
    await Promise.all(
