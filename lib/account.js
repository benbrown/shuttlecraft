...
export const deleteObject = async (actor, incomingRequest) => {
    ...
    try {
      ...
      if (activity.attributedTo != actor.id) { // only allow actor to delete their own Notes
          return false;
      }
    } catch (err) {
      ...
      if (inbox.some((m) => m.id === incomingRequest.object.id)) {
        ...
        writeJSONDictionary(inboxPath, inbox.filter((m) => {
...
export const createNote = async (body, cw, inReplyTo, toUser, editOf) => {
  ...
  // process recipients
  await Promise.all(to.concat(cc).map(async (recipient) => {
