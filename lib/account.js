 ...
 */
export const writeFollowers = followers => {
...
/**
 * The function `getFollowing` returns the contents of a JSON dictionary stored in the `followingFile`
 * variable.
 * @returns The function `getFollowing` is returning the result of calling the `readJSONDictionary`
 * function with the `followingFile` parameter.
 */
...
/**
 * The function checks if an account file exists and returns true if it does, otherwise it returns
 * false.
 * @returns a boolean value. It returns true if the accountFile exists, and false if it does not exist.
 */
export const ifAccount = () => {
  if (existsSync(accountFile)) {
    return true;
  }
  return false;
