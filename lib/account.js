 ...
 */
export const createActor = (name, domain, pubkey, bio) => {
  return {
...
export const updateAccount = async (name, domain, bio) => {
  // verify domain name
  const re = /^((?:(?:(?:\w[.\-+]?)*)\w)+)((?:(?:(?:\w[.\-+]?){0,62})\w)+)\.(\w{2,6})$/;
  if (!domain.match(re)) {
    console.error('DOMAIN setting "' + domain + '" does not appear to be a well-formatted domain name.');
  ...
  return new Promise((resolve, reject) => {
    ...
    crypto.generateKeyPair(
      ...
      (err, publicKey, privateKey) => {
        ...
        const actorRecord = createActor(name, domain, publicKey, bio);
        const webfingerRecord = createWebfinger(name, domain);
        const apikey = crypto.randomBytes(16).toString('hex');
        const account = {
          actor: actorRecord,
