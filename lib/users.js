...
import { sendActivityToFollowers } from './account.js';
const logger = debug('ono:users');

const { DOMAIN } = process.env;

export const fetchSigned = (targetUrl, options) => {
  ...
  let header = `keyId="${myaccount.actor.publicKey.id}",headers="(request-target) host date",signature="${signature_b64}"`;
  options.headers = {
    'Date': d.toUTCString(),
    'Host': url.hostname,
...
export const sendUndoFollowMessage = async (actorId, guid) => {
  const myaccount = getAccount();
  ...
  await sendToUser(actorId, message);
...
export const fetchOutbox = async(actor) => {
  if (actor.outbox) {
    try {
      const actorQuery = await fetchSigned(actor.outbox, {});
      if (actorQuery.ok) {
        ...
        if (rootOutbox.first) {
          if (typeof rootOutbox.first == 'string') {
            ...
            const pageQuery = await fetchSigned(rootOutbox.first, {});
            if (pageQuery.ok) {
              const outboxPage = await pageQuery.json();
              console.log('outbox page', outboxPage.orderedItems);
        ...
        items = await Promise.all(items.map(async (item) => {
          if (typeof item.object === 'string') {
            item.object = await getActivity(item.object);
          }
          return item;
        }));
  ...
  if (self) {
      ...
      try {
        ...
        const url = new URL(self);
          const actorQuery = await fetchSigned(self, {});
          if (actorQuery.ok) {
